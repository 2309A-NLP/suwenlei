...
QUESTION_TYPES = {  # 定义8种问句类型的模式字典，用于意图识别
    'entity': ['谁', '是谁', '哪家公司', '什么公司', '哪个公司', '是谁做的', '法定代表人', 'who', 'which company', 'whose', 'legal representative'],  # 实体查询类问句模式（中英文）
    'numeric': ['多少', '几个', '多少万', '多少钱', '多少元', '占比', '比重', '比例', 'how much', 'how many', 'what percentage', 'what ratio', 'what proportion'],  # 数值查询类问句模式（中英文）
    'list': ['哪些', '分别', '有哪些', '包括哪些', '分为哪些', '列举', 'which', 'what are', 'list', 'name'],  # 列举查询类问句模式（中英文）
    'confirm': ['是否', '是不是', '有没有', '是否属于', '是否包含', '有没有提到', 'is there', 'does it', 'is it', 'whether'],  # 确认验证类问句模式（中英文）
    'definition': ['什么是', '是什么', '什么叫', '如何理解', 'what is', 'what does', 'define', 'explain'],  # 定义解释类问句模式（中英文）
    'comparison': ['有什么区别', '有何不同', '比较', '对比', '与...相比', '相较于', 'compare', 'difference', 'versus', 'better'],  # 比较对比类问句模式（中英文）
    'procedure': ['如何', '怎么', '怎样', '步骤', '流程', 'how to', 'how does', 'what process', 'steps'],  # 流程步骤类问句模式（中英文）
    'time': ['什么时候', '何时', '哪一年', '报告期内', '最近', 'when', 'which year', 'during', 'recently'],  # 时间查询类问句模式（中英文）
}

# 招股说明书领域实体映射
# 优化：增加更多实体别名，覆盖10个评测问题的关键词
_ENTITY_ALIASES = {  # 定义领域实体别名映射，用于消歧处理
    ...
    # 英文别名映射，支持英文查询消歧
    'company': '武汉兴图新科电子股份有限公司',  # 英文简称映射为全称
    'Xingtuchuan': '武汉兴图新科电子股份有限公司',  # 拼音映射为全称
    'Wuhan Xingtuchuan': '武汉兴图新科电子股份有限公司',  # 拼音全称映射为全称
    'prospectus': '招股意向书',  # 英文映射为标准名称
    'military income': '来自军用领域的收入',  # 英文映射为标准表述
    'military revenue': '来自军用领域的收入',  # 英文映射为标准表述
    'defense customer': '国防客户',  # 英文映射为标准表述
    'video command': '视频指挥系统',  # 英文映射为完整系统名称
    'technical standard': '某视频技术规范1.0',  # 英文映射为具体规范名称
    'C4ISR': '某情报、指挥、控制与通信网络一体化工程',  # 英文缩写映射为中文全称
    'upstream enterprise': '电子信息行业的上游企业',  # 英文映射为领域特定表述
    'downstream industry': '电子信息行业的下游行业',  # 英文映射为领域特定表述
    'supplier': '国防军队视频指挥领域重要供应商',  # 英文映射为具体描述
    'legal representative': '法定代表人',  # 英文映射为标准法律术语
    'registered capital': '注册资本',  # 英文映射为具体财务术语
    'raised funds': '发行募集资金',  # 英文映射为完整表述
    'working capital': '补充流动资金',  # 英文映射为募资用途表述
    'military share': '军用收入占主营业务收入的比重',  # 英文映射为占比查询
    'main revenue': '主营业务收入',  # 英文映射为财务术语
    'first prize': '国家科技进步一等奖',  # 英文映射为奖项名称
    'IPO': '首次公开发行股票',  # 英文缩写映射为发行方式
    'Science and Technology Progress Award': '国家科技进步一等奖',  # 英文映射为奖项名称
}  # 优化：总条目从15增至60+，支持中英文查询
...
class QueryUnderstanding:
    ...
    def extract_keywords(query: str) -> List[str]:  # 从查询中提取关键词用于扩展
        ...
        # 中英文停用词集合，用于过滤无关词汇
        stop_words = {
            '一个', '这个', '那个', '什么', '怎么', '如何', '哪些', '多少', '分别', '根据', '报告', '期内', '过来', '一下', '我们', '你们', '他们', '自己', '可以', '没有',
        ...
        keywords = [w for w in words if len(w) >= 2 and w not in stop_words]  # 过滤停用词和单字词
    ...
    def expand_query(query: str) -> str:  # 优化：追加同义词（而非替换），提升检索召回率
        ...
        expansions = {  # 优化：定义查询扩展映射：原词→附加词列表
            ...
            # 英文同义词扩展，提升英文查询召回率
            'revenue': ['sales', 'income', 'earnings'],  # 收入相关英文扩展词
            'military': ['defense', 'armed forces', 'army'],  # 军用相关英文扩展词
            'upstream': ['supplier', 'raw material', 'procurement'],  # 上游相关英文扩展词
            'downstream': ['customer', 'application', 'industry'],  # 下游相关英文扩展词
            'technical standard': ['specification', 'standard', 'protocol'],  # 技术标准英文扩展词
            'registered capital': ['capital', 'equity', 'paid-in capital'],  # 注册资本英文扩展词
            'legal representative': ['representative', 'director'],  # 法人代表英文扩展词
            'raised funds': ['financing', 'fundraising', 'issuance'],  # 募集资金英文扩展词
            'working capital': ['liquidity', 'cash flow'],  # 流动资金英文扩展词
        }
