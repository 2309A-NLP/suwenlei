...
sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))  # 将项目根目录加入模块搜索路径
...
class KnowledgeBuilder:
    ...
    def build(self, pdf_path: str, chunk_size: int = 1000, chunk_overlap: int = 150):
        ...
        try:
            ...
            milvus_inserted = self._insert_to_milvus(chunks)  # 调用Milvus入库
    ...
    def _parse_pdf_to_csv(self, pdf_path: str, chunk_size: int, chunk_overlap: int) -> str:
        ...
        """
        try:
            from pdf_parser import PDFProcessor  # 导入PDF处理器
            ...
            result = processor.process(  # 执行PDF处理
    ...
    def _insert_to_milvus(self, chunks: list) -> int:
        """
        ...
        try:
            ...
            if not texts:  # 文本列表为空时
                logger.warning("[KB] 无文本内容，跳过Milvus入库")  # 记录警告
                return 0  # 返回0

            # 定义jieba分词函数（与 rag_pipeline 保持一致）
            stop_words = {  # 中文停用词表
                '的', '了', '在', '是', '我', '有', '和', '就', '不', '人',
                '都', '一', '上', '也', '很', '到', '说', '要', '去', '你',
                '会', '着', '没有', '看', '好', '自己', '这', '他', '她',
            ...
            }  # 与 rag_pipeline.py 一致的停用词表

            def jieba_tokenizer(text):  # 定义jieba分词函数
                words = list(jieba.cut(text))  # 对文本进行分词
                return [w for w in words if len(w) > 1 and w not in stop_words]  # 过滤单字词和停用词

            # 训练TF-IDF模型
            logger.info("[KB] 训练TF-IDF嵌入模型...")  # 打印训练日志
            tfidf = TfidfVectorizer(max_features=5000, tokenizer=jieba_tokenizer)  # 创建TF-IDF向量化器
            tfidf_mat = tfidf.fit_transform(texts)  # 拟合文档并转换得到TF-IDF矩阵

            # SVD降维到128维（与Milvus向量维度一致）
            svd = TruncatedSVD(n_components=128, random_state=42)  # 创建SVD降维器
            embeddings = svd.fit_transform(tfidf_mat).tolist()  # 降维并转为Python列表
            logger.info(f"[KB] TF-IDF({len(tfidf.vocabulary_)}词) → SVD(128维) 完成")  # 打印嵌入信息
            ...
            milvus.create_collection(force_recreate=True)  # 强制重建集合

            # 插入分块和向量
            logger.info(f"[KB] 向Milvus插入 {len(chunks)} 条分块...")  # 打印插入日志
            milvus.insert_chunks(chunks, embeddings)  # 批量插入分块和向量
