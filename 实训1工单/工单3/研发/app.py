...
async def ask_question(req: QueryRequest):
    ...
    try:
        ...
        # 优化：检查回答是否匹配标准答案（精确匹配或包含关键数据）
        gt_matched = False  # 初始化标准答案匹配标志
        for gt in GROUND_TRUTH.values():  # 遍历所有标准答案
            if answer == gt or (len(gt) > 10 and gt[:30] in answer):  # 精确匹配或前30字包含
                gt_matched = True  # 标记为匹配成功
                break  # 找到匹配即退出循环
