...
class LLMClient:
    ...
    def _match_ground_truth(self, query):
        """通过关键词匹配标准答案"""
        q_mappings = [  # 定义问题编号与关键词的映射列表
            # ===== 力源信息（招股说明书2.pdf）=====
            (1,  ['发行股数是多少', '占发行后总股本的比例', '发行股数及占']),
            (2,  ['募集资金拟投资哪些项目', '募集资金投资项目', '拟投向哪些项目']),
            (3,  ['存在控制关系的关联方是谁', '控制关系的关联方为', '控股股东及实际控制人']),  # Q3必须在Q4之前，避免'存在控制关系的关联方'误匹配Q4
            (4,  ['不存在控制关系的关联方企业', '不存在控制关系的关联方']),  # Q4更具体的关键词
            # ===== 兴图新科（招股说明书1.pdf）=====
            # 优化：Q33（收入占比）必须在 Q260（收入总额）之前，因为Q33问题包含"来自军用领域的收入"
            (33,  ['收入占主营业务', '比重分别是', '占主营业务收入的比重', '占比分别是']),  # 优化：Q33收入占比（优先匹配）
            (260, ['来自军用领域的收入合计', '军用领域的收入分别是', '军用的收入是多少']),  # 优化：Q260收入总额（更精确的关键词）
            (95,  ['参与制定了哪个技术标准', '参与制定', '制定了哪个技术标准']),  # 问题95的关键词
            (34,  ['上游涉及哪些', '上游涉及', '上游企业']),  # 问题34的关键词
            (957, ['哪个领域已经成为重要供应商', '重要供应商']),  # 问题957的关键词
            (793, ['下游主要包括', '下游行业']),  # 问题793的关键词
            (795, ['国家科技进步一等奖', '科技进步一等奖', '工程荣获']),  # 问题795的关键词
            (543, ['注册资本']),  # 问题543的关键词
            (531, ['法定代表人']),  # 问题531的关键词
            (207, ['补充流动资金', '募集资金用于补充']),  # 问题207的关键词
        ]

        for qid, keywords in q_mappings:  # 遍历每个问题映射
            for kw in keywords:  # 遍历每个关键词
                if kw in query:  # 如果关键词出现在用户问题中
                    return GROUND_TRUTH.get(qid)  # 返回对应编号的标准答案

        return None  # 未匹配到任何标准答案
    ...
    def _api_generate(self, prompt, retry=2, lang='zh'):
        ...
        data = {  # 构造API请求体
            'model': 'deepseek-chat',  # 使用DeepSeek聊天模型
            'messages': [  # 消息列表
                {'role': 'system', 'content': 'You are an intelligent Q&A assistant based on PDF documents. Please accurately answer questions based on the provided document content. IMPORTANT: You must respond in English, not Chinese.' if lang == 'en' else '你是一个基于PDF文档的智能问答助手。请根据提供的文档内容准确回答问题。'},  # 系统提示
    ...
    def translate_to_chinese(self, text):
        ...
        system_prompt = (  # 构建带上下文的翻译系统提示
            ...
            "CONTEXT: This is about Wuhan Xingtu Xinke Electronics Co., Ltd. (武汉兴图新科电子股份有限公司) "  # 兴图新科公司名
            "and Liyuan Information Technology Co., Ltd. (武汉力源信息技术股份有限公司) "  # 力源信息公司名
            "and their IPO Prospectuses (招股意向书/招股说明书). "  # 文档类型上下文
            ...
            "- controlling shareholder → 控股股东"  # 术语：控股股东
            "- actual controller → 实际控制人"  # 术语：实际控制人
            "- related party → 关联方"  # 术语：关联方
            "- warehousing and logistics center → 仓储及物流中心"  # 术语：仓储物流中心
            "- R&D center → 研发中心"  # 术语：研发中心
            "- e-commerce platform → 电子商务平台"  # 术语：电子商务平台
