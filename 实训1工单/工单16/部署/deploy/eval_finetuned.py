...
BASE_MODEL = "/root/autodl-tmp/models/Qwen/Qwen2-VL-2B-Instruct"
ADAPTER_PATH = "/root/RAG工单16/output"
...
def calc_bleu(ref, hyp):
    ...
    p = overlap / len(ht); bp = min(1.0, len(ht) / max(len(rt), 1))
    return bp * p
...
def main():
    ...
    print("加载基座模型+LoRA adapter...")
    processor = AutoProcessor.from_pretrained(BASE_MODEL, trust_remote_code=True)
    model = Qwen2VLForConditionalGeneration.from_pretrained(BASE_MODEL, torch_dtype=torch.bfloat16, device_map="auto")
    ...
    with open(os.path.join(OUTPUT_DIR, "finetuned_results.json"), 'w') as f: json.dump(results, f, ensure_ascii=False, indent=2)
    print(f"\n=== 微调后评估 ===")
