...
def _tokenize(text: str) -> List[str]:
    ...
    try:
        ...
        words = list(jieba.cut(text))       # jieba精确模式分词
    except ImportError:
        # jieba不可用时降级为正则切分（中文词或英文数字串）
        words = re.findall(r'[\u4e00-\u9fff]{2,}|[a-zA-Z0-9]+', text)
    return [w for w in words if len(w) >= 1 and w not in STOP_WORDS]  # 过滤单字和停用词
...
class MilvusStore:
    ...
    def connect(self):
        ...
        try:
            ...
            logger.warning(f"[Milvus] 连接失败，降级BM25: {e}")
