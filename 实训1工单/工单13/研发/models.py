...
class EmbeddingClient:
    ...
    def encode(self, texts: list[str], batch_size: int = 32) -> np.ndarray:  # 批量编码文本列表
        """编码文本列表，返回 shape=(N, EMBEDDING_DIM) 的 numpy 数组"""
        self._load_model()                      # 确保模型已加载
        embeddings = self._model.encode(
            texts,                              # 输入文本列表
            batch_size=batch_size,              # 每批处理数量
            show_progress_bar=False,            # 不显示进度条
            normalize_embeddings=True           # L2 归一化
        )
        return np.array(embeddings, dtype=np.float32)  # 转为 float32 数组
...
class RerankerClient:
    ...
    def rerank(self, query: str, documents: list, top_k: int = 10, max_length: int = 128) -> list[dict]:  # 重排序(max_length优化: 256→128减少tokenize开销)
