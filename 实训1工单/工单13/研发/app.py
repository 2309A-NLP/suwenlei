...
from monitoring import (
    RequestTrace, trace_store, alert_store,
    REQUEST_COUNT, ACTIVE_REQUESTS, RETRIEVAL_RESULTS, LLM_TOKENS,
    get_metrics, get_metrics_content_type, ALERT_THRESHOLDS
)
...
async def ask_question(req: QueryRequest):
    ...
    ACTIVE_REQUESTS.inc()
    rt = RequestTrace(endpoint="/api/ask")
    try:
        engine, llm = get_engine()
        start = time.time()
        question = req.question.strip()
        ...
        if not question:
            REQUEST_COUNT.labels(endpoint="/api/ask", status="error").inc()
            return JSONResponse(
        ...
        with rt.stage("query_understanding"):
            qu = QueryUnderstanding.understand(question)
        ...
        if not context:
            ...
            trace_summary = rt.finish(status="no_context")
            trace_store.add(trace_summary)
            return {
                'question': question,
                'answer': answer,
                'sources': [],
        ...
        rt.metadata['question'] = question
        rt.metadata['intent'] = qu['intent']
        trace_summary = rt.finish(status="success")
        trace_store.add(trace_summary)
        ...
        trace_summary = rt.finish(status="error")
        trace_store.add(trace_summary)
        return JSONResponse(
        ...
        trace_summary = rt.finish(status="error")
        trace_store.add(trace_summary)
        alert_store.add("system_error", str(e), rt.trace_id)
        return JSONResponse(
            content={
                'error': '系统内部错误，请稍后重试',
...
async def ask_question_stream(req: QueryRequest):
    ...
    conv_id = req.conversation_id or _store.generate_id()
    history = _store.get_history(conv_id)
    target_lang = req.lang if req.lang in ('en', 'zh') else 'zh'
    question_for_index = llm.translate(question, 'zh') if target_lang == 'en' else question
    ...
    retrieve_time = round(t_retrieve_end - t_retrieve_start, 3)
    ...
    if not context:
        ...
        _store.append_message(conv_id, 'user', question)
        _store.append_message(conv_id, 'assistant', answer)
        meta = _json.dumps({'type': 'meta', 'sources': sources, 'retrieve_time': retrieve_time,
                           'conversation_id': conv_id, 'intent': qu['intent']}, ensure_ascii=False)
        return StreamingResponse(iter([f"data: {meta}\n\n", f"data: {_json.dumps({'type': 'done', 'answer': answer})}\n\n"]),
                                media_type="text/event-stream")

    prompt = engine.build_prompt(question_for_index, context, history=history, lang=target_lang)
    generator, is_stream = llm.generate_stream(prompt, context, question_for_index, history=history)

    def event_stream():
        full_answer = []
        # 先发送元数据（包含检索耗时和sources）
        meta = _json.dumps({'type': 'meta', 'sources': sources, 'retrieve_time': retrieve_time,
                           'conversation_id': conv_id, 'intent': qu['intent']}, ensure_ascii=False)
        yield f"data: {meta}\n\n"
        ...
        if target_lang == 'en' and _is_chinese_text(answer_text):
            answer_text = llm.translate(answer_text, 'en')
            yield f"data: {_json.dumps({'type': 'chunk', 'content': answer_text}, ensure_ascii=False)}\n\n"

        _store.append_message(conv_id, 'user', question)
        _store.append_message(conv_id, 'assistant', answer_text)

        elapsed = round(time.time() - start, 2)
        yield f"data: {_json.dumps({'type': 'done', 'total_time': elapsed, 'retrieve_time': retrieve_time, 'answer': answer_text}, ensure_ascii=False)}\n\n"
        logger.info(f"[流式问答] 检索={retrieve_time:.3f}s 总计={elapsed:.2f}s 问题='{question[:30]}'")
...
async def get_trace_detail(trace_id: str):
    """获取单个追踪详情"""
    traces = trace_store.get_recent(1000)
    for t in traces:
...
async def get_alerts(limit: int = 50):
    """获取最近的告警记录"""
    alerts = alert_store.get_recent(limit)
    return {
        'alerts': alerts,
        'total': len(alerts)
    }
...
async def monitoring_dashboard():
    """监控仪表盘数据"""
    stats = trace_store.get_stats()
    recent_traces = trace_store.get_recent(10)
    recent_alerts = alert_store.get_recent(10)

    # 各阶段平均耗时
    stage_stats = {}
    for t in trace_store.get_recent(100):
        for stage, duration in t.get('stages', {}).items():
            if stage not in stage_stats:
                stage_stats[stage] = []
            stage_stats[stage].append(duration)

    stage_avg = {
        stage: round(sum(durations) / len(durations), 4)
        for stage, durations in stage_stats.items()
    }

    return {
        'overview': stats,
        'stage_avg_latency': stage_avg,
...
def main():
    ...
    print(f"[启动] 后续请求检索时间预计 < 1s (不含LLM生成)")
    print()
