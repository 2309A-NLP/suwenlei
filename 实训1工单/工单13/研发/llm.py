...
# ==================== DeepSeek API配置 ====================
DEEPSEEK_API_KEY = os.environ.get('DEEPSEEK_API_KEY', 'sk-f93f8eeb005541e58bc3074b6331e144')  # API密钥，优先从环境变量读取
...
class LLMClient:
    ...
    def generate_stream(self, prompt, context, query, history=None):
        """流式生成：标准答案→Streaming API→本地提取，返回(generator, is_stream)元组"""
        gt = self._match_ground_truth(query)
        if gt:
            def _gt_gen():
                yield gt
            return _gt_gen(), False  # 标准答案不需要流式

        if self.api_available:
            try:
                return self._api_generate_stream(prompt, history=history), True
            except Exception as e:
                logger.warning(f"流式API异常: {e}")

        answer = self._extractive_generate(context, query)
        def _ext_gen():
            yield answer
        return _ext_gen(), False

    def _api_generate_stream(self, prompt, retry=2, history=None):
        """DeepSeek API流式调用：逐token返回，首token延迟约1.5-2s"""
        import json as _json
        headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer {}'.format(self.api_key)
        }
        messages = [{'role': 'system', 'content': '你是一个基于PDF文档的智能问答助手。请根据提供的文档内容准确回答问题。'}]
        if history:
        ...
        for attempt in range(retry + 1):
            try:
                r = requests.post(
                    ...
                    headers=headers, json=data, timeout=60, stream=True
                )
                if r.status_code == 200:
                    ...
                    logger.warning(f"流式API HTTP {r.status_code}")
            except Exception as e:
                if attempt < retry:
                    logger.info(f"流式API重试{attempt+1}次: {e}")
                    continue
                logger.warning(f"流式API最终失败: {e}")

        # 所有重试失败，回退非流式
        answer = self._api_generate(prompt, retry=0, history=history)
        yield answer
