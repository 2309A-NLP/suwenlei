...
from llm import LLMClient, QueryUnderstanding, DEEPSEEK_BASE_URL
...
class RAGEngine:
    ...
    def _route_query(self, query: str) -> List[str]:
        """智能路由：根据问题自动选择最相关的知识库"""
        all_docs = list(self.doc_engines.keys())
        if len(all_docs) <= 1:
            # 只有一个或没有文档，无需路由
            return all_docs

        # 构建文档摘要：每个文档取前200字符作为描述
        doc_descriptions = []
...
请返回与问题相关的文档名称（多个用逗号分隔），如果所有文档都相关就返回"ALL"。只返回文档名，不要解释。"""
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if content.upper() == 'ALL':
                    logger.info(f"[LLM路由] '{query[:30]}...' → 全部文档")
                    return all_docs
                # 解析返回的文档名
                matched = []
                for name in content.split(','):
                    name = name.strip()
                    if name in self.doc_engines:
                        matched.append(name)
                if matched:
                    logger.info(f"[LLM路由] '{query[:30]}...' → {matched}")
                    return matched
        except Exception as e:
            logger.warning(f"[LLM路由] 失败，回退全文档搜索: {e}")

        # LLM路由失败时搜索所有文档
        logger.info(f"[路由] '{query[:30]}...' → 全文档搜索 ({len(all_docs)}个)")
        return all_docs
    ...
    def build_prompt(self, question: str, context: str, history: list = None, lang: str = 'zh') -> str:
        """构建LLM Prompt：注入对话历史+检索上下文，支持中英文多语言输出"""
        if not context:
        ...
        return (
            ...
            f"3. If the question is a follow-up (e.g. 'this company', '那XX呢', '它的XX'), use conversation history to understand context.\n"
            f"4. Retrieved content types:\n"
            f"   [text] Text paragraphs\n"
            f"   [table] Table data\n"
            f"   [图片] Image descriptions — text descriptions of charts/figures in the PDF\n\n"
            f"Retrieved document content:\n{context}\n\n"
            f"User question: {question}\n\n"
            f"Please provide an accurate, concise answer based on the above documents. Cite specific data from the source when available."
        )
