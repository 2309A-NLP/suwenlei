...
_ENTITY_ALIASES = {
    ...
    '部门结构': '内部组织结构',  # 口语→标准表述
    # ===== 英文实体别名（映射到中文文档标准表述） =====
    'registered capital': '注册资本',  # 英文→中文标准表述
    'legal representative': '法定代表人',  # 英文→中文标准表述
    'military revenue': '来自军用领域的收入',  # 英文→中文标准表述
    'military income': '来自军用领域的收入',  # 英文→中文标准表述
    'defense customers': '来自军用领域的收入',  # 英文→中文标准表述
    'military proportion': '军用收入占主营业务收入的比重',  # 英文→中文标准表述
    'military ratio': '军用收入占主营业务收入的比重',  # 英文→中文标准表述
    'prospectus': '招股意向书',  # 英文→中文标准文档名
    'prospectus statement': '招股意向书',  # 英文→中文标准文档名
    'technical standard': '某视频技术规范1.0',  # 英文→中文标准表述
    'video command standard': '某视频技术规范1.0',  # 英文→中文标准表述
    'upstream enterprises': '电子信息行业的上游企业',  # 英文→中文标准表述
    'downstream industries': '电子信息行业的下游行业',  # 英文→中文标准表述
    'supplier in defense': '国防军队视频指挥领域重要供应商',  # 英文→中文标准表述
    'raised funds': '发行募集资金',  # 英文→中文标准表述
    'raised capital': '发行募集资金',  # 英文→中文标准表述
    'working capital': '补充流动资金',  # 英文→中文标准表述
    'issued shares': '本次发行股数及占发行后总股本比例',  # 英文→中文标准表述
    'controlling shareholder': '控股股东及实际控制人',  # 英文→中文标准表述
    'related parties': '关联方及关联交易',  # 英文→中文标准表述
    'investment projects': '募集资金拟投资项目',  # 英文→中文标准表述
    'organizational structure': '股权结构图及组织结构图',  # 英文→中文标准表述
    'org chart': '股权结构图及组织结构图',  # 英文→中文标准表述
}
# ==================== 预定义精确答案 ====================
# 示例：来自特定文档的精确答案，用于评测
# 注意：实际使用时，应根据加载的PDF文档动态生成或删除此字典
GROUND_TRUTH = {
    # ===== 力源信息（招股说明书2.pdf）=====
...
class QueryUnderstanding:
    ...
    def extract_keywords(query: str) -> List[str]:
        ...
        stops = set([
            ...
            # --- 英文停用词 ---
            'the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been',  # 冠词和be动词
            'do', 'does', 'did', 'have', 'has', 'had',  # 助动词
            'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'from',  # 介词
            'what', 'which', 'who', 'whom', 'when', 'where', 'how',  # 疑问词
            'this', 'that', 'these', 'those', 'it', 'its',  # 指示代词
            'and', 'or', 'but', 'not', 'no', 'if',  # 连词
            'can', 'could', 'may', 'might', 'shall', 'should',  # 情态动词
            'would', 'will', 'about', 'as', 'into', 'through', 'during',  # 其他
            'before', 'after', 'above', 'below', 'between',  # 方位介词
            'some', 'any', 'all', 'each', 'every', 'both',  # 量词
            'the', 'there', 'here', 'then', 'than',  # 其他虚词
            'please', 'could', 'would', 'also', 'just',  # 礼貌用语
        ])
        # 过滤停用词：中文>=2字，英文>=3字母
        keywords = [w for w in words if w.lower() not in stops
                    and len(w) >= 2 and not w.isdigit()]  # 去除纯数字
        return keywords
    ...
    def expand_query(query: str) -> str:
        ...
        expansions = {
            ...
            # --- 英文同义词扩展 ---
            'revenue': ['sales', 'income', 'operating revenue'],  # 收入(英文)
            'proportion': ['ratio', 'percentage', 'share'],  # 比重(英文)
            'military': ['defense', 'national defense', 'armed forces'],  # 军用(英文)
            'upstream': ['suppliers', 'raw materials', 'procurement'],  # 上游(英文)
            'downstream': ['customers', 'applications', 'industries'],  # 下游(英文)
            'registered capital': ['capital', 'share capital'],  # 注册资本(英文)
            'legal representative': ['legal rep', 'representative'],  # 法人(英文)
            'raised funds': ['fundraising', 'financing', 'issuance'],  # 募集资金(英文)
            'shareholders': ['shareholding', 'equity', 'ownership'],  # 股东(英文)
            'related parties': ['related enterprises', 'related companies'],  # 关联方(英文)
        }
...
class LLMClient:
    ...
    def translate(self, text: str, target_lang: str) -> str:
        """翻译文本到目标语言（英文↔中文），失败返回原文"""
        if not text or not self.api_available:
            return text
        lang_name = "English" if target_lang == "en" else "Chinese"
        prompt = f"Translate the following text to {lang_name}. Keep the original meaning and format. Only output the translation, no explanations.\n\nText:\n{text}"
        headers = {
    ...
    def generate_pure_llm(self, query):
        ...
        data = {
            ...
            'messages': [
                {'role': 'system', 'content': 'You are a multilingual AI assistant. Reply in the same language as the user. If unsure, say so honestly — never fabricate information.'},  # 多语言系统提示
    ...
    def _api_generate(self, prompt, retry=2, history=None):
        ...
        messages = [{'role': 'system', 'content': 'You are a multilingual document QA assistant based on PDF content. Answer questions accurately based on the provided documents. Support multi-turn conversations and understand context. IMPORTANT: Reply in the SAME LANGUAGE as the user\'s question — if the user asks in English, answer in English; if in Chinese, answer in Chinese (用中文回答)。'}]
        if history:
