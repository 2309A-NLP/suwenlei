...
class DocumentEngine:
    ...
    def __init__(self, doc_name: str, csv_path: str):
        ...
        self.tfidf_mat = None  # 全量TF-IDF矩阵
    ...
    def _init_embedder(self):
        ...
        try:
            ...
            def jieba_tokenizer(text):  # 定义jieba分词函数
                words = list(jieba.cut(text))  # 对文本进行分词
                return [w for w in words if len(w) > 1 and w not in CHINESE_STOP_WORDS]  # 过滤单字词和停用词

            self.tfidf = TfidfVectorizer(max_features=5000, tokenizer=jieba_tokenizer)  # 创建TF-IDF向量化器
            self.tfidf_mat = self.tfidf.fit_transform(texts)  # 拟合文档并转换得到TF-IDF矩阵
            self.svd = TruncatedSVD(n_components=128, random_state=42)  # 创建SVD降维器
            self.svd.fit(self.tfidf_mat)  # 拟合SVD模型
            logger.info(f"[DocEngine] {self.doc_name}: TF-IDF({len(self.tfidf.vocabulary_)}词) -> SVD(128维)")
        except Exception as e:  # 训练失败时
            logger.error(f"[DocEngine] {self.doc_name} 嵌入模型训练失败: {e}")  # 记录错误
...
class RAGEngine:
    ...
    def get_context(self, query: str, top_k=25, original_query: str = None) -> Tuple[str, List[Dict]]:
        ...
        for doc_name in target_docs:  # 遍历目标文档
            ...
            if self.milvus_store and self.milvus_store.is_available():  # Milvus可用时
                try:
                    query_emb = embed_query(query)  # 用bge-m3生成4096维查询向量
                    if query_emb:  # 向量生成成功时
                        hits = self.milvus_store.search(query_emb, top_k * 3, doc_name=doc_name)  # 搜索Milvus（候选数3倍）
                        if hits:  # 检索到结果时
        ...
        # 6. 确保关键词补充结果不被Milvus高分结果挤掉
        keyword_set = {f"{r.get('page_num', '')}_{r.get('text', '')[:50]}" for r in (keyword_boost or [])}
        kw_in_top = [r for r in unique_results[:top_k] if f"{r.get('page_num', '')}_{r.get('text', '')[:50]}" in keyword_set]
        kw_not_in_top = [r for r in unique_results if f"{r.get('page_num', '')}_{r.get('text', '')[:50]}" in keyword_set and r not in unique_results[:top_k]]

        if kw_not_in_top and len(kw_in_top) < len(kw_not_in_top):
            # 用关键词结果替换top_k中排名最低的非关键词结果
            mixed = unique_results[:top_k]
            non_kw = [r for r in mixed if f"{r.get('page_num', '')}_{r.get('text', '')[:50]}" not in keyword_set]
            for kr in kw_not_in_top[:len(non_kw)]:
                if non_kw:
                    worst = non_kw.pop()
                    mixed.remove(worst)
                    mixed.append(kr)
            mixed.sort(key=lambda r: r['score'], reverse=True)
            unique_results = mixed + [r for r in unique_results[top_k:] if r not in mixed]
    ...
    def _keyword_supplement_search(self, query: str, target_docs: List[str]) -> List[Dict]:
        """
        关键词补充检索：从查询中提取关键短语，直接在CSV中搜索包含这些短语的chunk
        解决向量搜索遗漏包含精确数据的chunk的问题（如图表中的行业增长率数据）
        """
        # 关键词列表：越长越精准，权重越高
        KEYWORD_WEIGHTS = {
            '补充流动资金': 3.0, '募集资金用途': 3.0, '募集资金投资': 3.0,
            '上游行业企业': 3.0, '上游涉及': 3.0, '上游企业': 3.0,
            '下游行业': 3.0, '下游主要包括': 3.0, '下游客户': 2.5,
            '电子信息行业': 2.5, '电子元器件': 2.0,
            '军用领域': 2.5, '国防客户': 2.5, '军方客户': 2.5,
            '主营业务收入': 2.0, '注册资本': 2.5, '法定代表人': 2.5,
            '国家科技进步': 3.0, '技术标准': 2.5, '重要供应商': 2.5,
            'IC市场': 2.0, '应用结构': 2.0, '市场应用': 2.0,
            '增长率': 1.5, '领域': 1.5, '行业': 1.0,
            '条形图': 1.5, '图表': 1.5, '数据': 1.0, '比例': 1.5, '占比': 1.5,
            '负增长': 1.5, '最高': 1.0, '最快': 1.0, '最低': 1.0,
        }
        # 从查询中提取匹配的关键词并计算权重
        matched = [(kw, w) for kw, w in KEYWORD_WEIGHTS.items() if kw in query]
        if not matched:
            return []
        ...
        for doc_name in target_docs:
            ...
            for chunk in engine.chunks:
                ...
                if text_prefix in seen_texts:
                    continue
                # 计算加权匹配分数
                weighted_score = sum(w for kw, w in matched if kw in text)
                if weighted_score >= 1.5:  # 加权阈值
                    seen_texts.add(text_prefix)
                    boost_score = min(0.2 + 0.1 * weighted_score, 0.9)
        ...
        # 按分数排序，取top30避免噪音
        supplement_results.sort(key=lambda r: r['score'], reverse=True)
        supplement_results = supplement_results[:30]

        if supplement_results:
            logger.info(f"[关键词补充] 查询 '{query[:30]}...' -> 补充 {len(supplement_results)} 条结果")
