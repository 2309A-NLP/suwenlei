...
"""
RAG引擎核心模块
依赖 pdf_processor.py 进行PDF解析（去水印/页眉页脚，提取表格和图片）
...
class RAGEngine:
    ...
    def init_rag(self):
        ...
        self.chunks = self.parse_pdf_to_chunks()  # 解析PDF并获取分块数据
        if self.chunks:  # 分块不为空时
            max_page = max(c.get('page_num', 0) for c in self.chunks)  # 获取最大页码
            logger.info(f"PDF解析完成: {max_page} 页, {len(self.chunks)} 块")  # 打印解析统计
        else:  # 分块为空时
            logger.warning("PDF解析为空")  # 记录警告
        self.init_embedder()  # 初始化嵌入模型

    def load_pdf(self, pdf_path: str):
        self.pdf_path = pdf_path  # 设置PDF路径
        self.init_rag()  # 初始化RAG引擎

    # ---- PDF解析（优先从CSV加载，避免重复解析） ----
    def parse_pdf_to_chunks(self) -> List[Dict]:
        """从CSV加载分块数据，CSV不存在时才解析PDF"""
        ...
        # 2. CSV不存在或加载失败，解析PDF
        logger.info("开始解析PDF（首次运行或CSV不存在）...")  # 打印解析开始日志
        try:
            from pdf_processor import PDFProcessor  # 导入PDF处理器
            with PDFProcessor(self.pdf_path) as processor:  # 打开PDF处理器
                chunks, _ = processor.process_all()  # 完整处理PDF
            result = []  # 初始化结果列表
            for c in chunks:  # 遍历每个分块
                prefix = ""  # 前缀初始化为空
                if c.source_type == "table":  # 表格类型
                    prefix = "[表格] "  # 添加表格标记
                elif c.source_type == "image":  # 图片类型
                    prefix = "[图片] "  # 添加图片标记
                result.append({  # 将分块转为字典格式
                    "content": prefix + c.text,  # 内容加前缀标记
                    "page_num": c.page_num,  # 页码
                    "chunk_idx": c.id,  # 分块索引
                    "source_type": c.source_type,  # 来源类型
                })
            # 保存到CSV供后续使用
            try:
                processor.export_to_csv(chunks, csv_path)  # 导出CSV文件
                logger.info(f"解析结果已保存到: {csv_path}")  # 打印保存日志
            except Exception as e:  # CSV保存失败不影响运行
                logger.warning(f"CSV保存失败（不影响运行）: {e}")  # 记录警告
            return result  # 返回解析结果
        except ImportError:  # pdf_processor模块未找到
            logger.warning("pdf_processor 未找到，回退到基础解析")  # 记录警告
            return self._fallback_parse()  # 使用回退解析
        except Exception as e:  # 其他解析异常
            logger.error(f"pdf_processor 解析失败: {e}，使用回退解析")  # 记录错误
            return self._fallback_parse()  # 使用回退解析
    ...
    def init_embedder(self):
        try:
            ...
            def jieba_tokenizer(text):  # 定义jieba分词函数
                return list(jieba.cut(text))  # 对文本进行分词并返回词列表
    ...
    def _tfidf_search(self, query: str, top_k=5) -> Tuple[str, List[Dict]]:
        ...
        # 获取 top_k 索引
        top_indices = scores.argsort()[::-1][:top_k]  # 按相似度降序排列，取前top_k个索引
        results = []  # 初始化结果列表
        for idx in top_indices:  # 遍历每个高评分索引
            if scores[idx] <= 0:  # 评分为0或负数时跳过
                continue
            c = self.chunks[idx]  # 获取对应的分块
            results.append({  # 添加结果字典
                "text": c["content"],  # 文本内容
                "page_num": c["page_num"],  # 页码
                "score": round(float(scores[idx]), 4),  # 相似度评分，保留4位小数
            })
    ...
    # ---- 主问答 ----
    def query(self, user_query: str, top_k=5) -> Dict:
        """完整问答流程"""
